...
def run_svg_api_check(prompt: str):
        ...
        if response.status_code == 200:
            ...
            if "history" in result:
                ...
                for i, entry in enumerate(history):
                    ...
                    print(
                        f"    - VQA 状态: {entry.get('vqa_results', {}).get('status')}"
                    )
...
if __name__ == "__main__":
    run_svg_api_check(
        "A modern, minimalist network architecture diagram showing 3 layers: Core, Aggregation, and Access."
    )
