...
from src.agents.asset_management.processors.audit import (
    sanitize_svg,
...
async def test_audit_svg_visual_async_parsing_success():
    ...
    with patch(
        "src.agents.asset_management.processors.audit.render_svg_with_playwright",
    ...
    assert (
        "[Unified Audit] The SVG looks good but has one overlap." in mock_state.thoughts
    )
    assert "[Audit Score] 88/100" in mock_state.thoughts
...
async def test_audit_svg_visual_async_fallback():
    ...
    with patch(
        ...
        with patch(
            "src.agents.asset_management.processors.audit.render_svg_to_png_base64",
...
async def test_audit_svg_visual_async_with_state_capture():
    ...
    mock_response.success = True
    mock_response.text = (
        '{"thought": "Deep thought", "overall_score": 90, "issues": []}'
    )
    ...
    assert "[Unified Audit] Deep thought" in state.thoughts
    assert "[Audit Score] 90/100" in state.thoughts
