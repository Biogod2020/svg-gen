...
def agent_state(tmp_path):
    state = AgentState(job_id="test-job", workspace_path=str(tmp_path))
    return state


@pytest.mark.asyncio
async def test_run_optimization_loop_captures_history(
    mock_gemini_client, agent_state, tmp_path
):
    ...
    with patch(
        ...
        with patch(
            "src.agents.svg_generation.agent.generate_svg_async", new_callable=AsyncMock
        ) as mock_gen:
            mock_gen.return_value = "<svg viewBox='0 0 100 100'>Initial</svg>"

            # Mock audit_svg_visual_async
            with patch(
                "src.agents.svg_generation.agent.audit_svg_visual_async",
                ...
                with patch(
                    "src.agents.svg_generation.agent.repair_svg_async",
                    new_callable=AsyncMock,
                ) as mock_repair:
                    mock_repair.return_value = (
                        "<svg viewBox='0 0 100 100'>Repaired</svg>"
                    )
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            with patch.object(
                                AgentState, "get_uar", return_value=mock_uar
                            ):
                                all_iterations = []
                                ...
                                iterations = [
                                    i for i in all_iterations if i["iteration"] > 0
                                ]
                                ...
                                # Check first entry
                                assert (
                                    iterations[0]["svg_code"]
                                    == "<svg viewBox='0 0 100 100'>Initial</svg>"
                                )
                                assert (
                                    iterations[0]["vqa_results"].status
                                    == AssetVQAStatus.FAIL
                                )
                                ...
                                # Check second entry
                                assert (
                                    iterations[1]["svg_code"]
                                    == "<svg viewBox='0 0 100 100'>Repaired</svg>"
                                )
                                assert (
                                    iterations[1]["vqa_results"].status
                                    == AssetVQAStatus.PASS
                                )
...
async def test_optimize_svg_pipeline_returns_history(mock_gemini_client, tmp_path):
    ...
    async def mock_gen(*args, **kwargs):
        yield {
            "svg_code": "<svg></svg>",
            "vqa_results": MagicMock(spec=AuditResult),
            "png_b64": "...",
            "thoughts": "...",
            "iteration": 1,
        }
