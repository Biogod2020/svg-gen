...
async def test_run_optimization_loop_generator(tmp_path):
    ...
    with patch("src.agents.svg_generation.agent.RefinementProcessor") as mock_refiner_cls, \
         ...
         patch("src.agents.svg_generation.agent.refine_caption_async") as mock_refine_caption, \
         patch("src.agents.svg_generation.agent.render_svg_to_png_base64") as mock_render:
        
        # Setup mocks
        mock_refiner = mock_refiner_cls.return_value
        mock_refiner.refine_request_async = AsyncMock(return_value=PreflightBlueprint(
            refined_task_directive="Refined",
            specific_style_hints="Style",
            dynamic_audit_checkpoints=[]
        ))
        
        mock_generate.return_value = "<svg>initial</svg>"
